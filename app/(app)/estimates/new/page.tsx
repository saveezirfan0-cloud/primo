...
// Brief parsing is a Claude call from the startEstimate server action.
...
export default async function NewEstimatePage({ searchParams }: PageProps<"/estimates/new">) {
  ...
  const error = sp.error === "short" ? "Write a few sentences describing the job." : typeof sp.error === "string" && sp.error ? `Could not parse the brief: ${sp.error}. Your text is kept below; try again.` : null;
          ...
          <CardContent>
            <form action={startEstimate} className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea id="brief" name="brief" rows={9} defaultValue={sp.demo === "1" ? DEMO_BRIEF : typeof sp.brief === "string" ? sp.brief : ""} placeholder={DEMO_BRIEF} required />
              </div>
